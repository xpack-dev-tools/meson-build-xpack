...
**@xpack-dev-tools/meson-build** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
```sh
rm -rf ~/Work/xpack-dev-tools/meson-build-xpack.git/package-lock.json
```
...
npm --prefix ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
...
(such as **1.8.5-1.1**) with the following expression:
...
xpm run trigger-workflow-test-docker-linux-arm64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
tagged such as **v1.8.5-1** (mind the dash in the middle!) and
named such as **xPack Meson Build v1.8.5-1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 1.8.5-1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/meson-build-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
