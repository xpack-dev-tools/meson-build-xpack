...
# How to make new xPack Meson Build releases
...
[![GitHub pulls](https://img.shields.io/github/issues-pr/xpack-dev-tools/meson-build-xpack)](https://github.com/xpack-dev-tools/meson-build-xpack/pulls)
...
- determine the version (like <code>1.5.2-1.1</code>)
- update the version in top `package.json`
- use the new version, suffixed by `.pre`,
like <code>1.5.2-1.1.pre</code>.
...
Determine the version (like <code>1.5.2-1</code>)
and update the `build-assets/scripts/VERSION` file; the format
is <code>1.5.2-1</code>.
...
and fix them; assign them to a milestone (like <code>1.5.2-1</code>).
...
- add a new entry like _**\* v1.5.2-1 prepared**_
- commit with the message _**prepare v1.5.2-1**_
...
```sh
rm -f ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/meson-build-xpack.git pull

xpm run install -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets

git -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git pull
xpm link -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git

xpm run link-deps -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets

xpm run deep-clean --config darwin-x64  -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets && \
xpm install --config darwin-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets && \
xpm run build-development --config darwin-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```

For a debug build:

```sh
xpm run build-development-debug --config darwin-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
(like **1.5.2**) with the following expression:
...
```sh
xpm run check-space -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
```sh
xpm run clear-all-projects-builds -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
```sh
xpm run generate-workflows -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-build-darwin-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-build-darwin-arm64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-build-linux-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-build-win32-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-build-linux-arm64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-build-linux-arm -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
[GitHub Actions](https://github.com/xpack-dev-tools/meson-build-xpack/actions) page.
...
```sh
xpm run trigger-workflow-test-prime -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-publish-release -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
tagged like **v1.5.2-1** (mind the dash in the middle!) and
named like **xPack Meson Build v1.5.2-1** (mind the dash),
...
watching the meson-build-xpack project.
...
```sh
xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```

- edit the front matter properties
- select the correct summary
- commit with the message _**website: blog post release 1.5.2-1 published**_
...
```sh
xpm run update-package-binaries -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
- commit all changes with the message
  _**package.json: update urls for 1.5.2-1.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v1.5.2-1.1 published on npmjs.com**_
- commit with the message _**CHANGELOG: publish npm v1.5.2-1.1**_
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/meson-build?activeTab=versions">npmjs.com</a>.
...
```sh
xpm run trigger-workflow-test-xpm -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
<CodeBlock language="sh"> {
`npm dist-tag add @xpack-dev-tools/meson-build@1.5.2-1.1 latest
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/meson-build@1.5.2-1.1
`} </CodeBlock>
...
in https://xpack-dev-tools.github.io/meson-build-xpack/blog/
- remember the post URL, since it must be used to update the release page
and the X/Twitter announcement
- switch to https://xpack-dev-tools.github.io/meson-build-xpack/docs/maintainer/#update-the-xpack-branch
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/meson-build-xpack.git/build-assets
```
...
- paste the release name like _**xPack Meson Build v1.5.2-1 released**_
...
[![Github All Releases](https://img.shields.io/github/downloads/xpack-dev-tools/meson-build-xpack/latest/total)](https://github.com/xpack-dev-tools/meson-build-xpack/releases)
...
[![npm (scoped)](https://img.shields.io/npm/v/@xpack-dev-tools/meson-build)](https://www.npmjs.com/package/@xpack-dev-tools/meson-build/)
[![npm](https://img.shields.io/npm/dm/@xpack-dev-tools/meson-build)](https://www.npmjs.com/package/@xpack-dev-tools/meson-build/)
  - all releases [![npm](https://img.shields.io/npm/dt/@xpack-dev-tools/meson-build)](https://www.npmjs.com/package/@xpack-dev-tools/meson-build/)
