...
Determine the version (like `1.1.1`) and update the `scripts/VERSION`
file; the format is `1.1.1-1`. The fourth number is the xPack release number
...
and fix them; assign them to a milestone (like `1.1.1-1`).
...
Use the new version, suffixed by `pre`, like `1.1.1-1.1.pre`.

### Update `CHANGELOG.md`

- open the `CHANGELOG.md` file
- check if all previous fixed issues are in
- add a new entry like _* v1.1.1-1 prepared_
- commit with a message like _prepare v1.1.1-1_
...
total 19924
-rw-r--r-- 1 ilg ilg 20395664 Jan 28 11:19 xpack-meson-build-1.1.1-1-linux-arm.tar.gz
-rw-r--r-- 1 ilg ilg      110 Jan 28 11:19 xpack-meson-build-1.1.1-1-linux-arm.tar.gz.sha
```
...
```sh
.../xpack-meson-build-1.1.1-1/bin/meson-build --version
1.1.1
```
...
- in `CHANGELOG.md`, add the release date and a message like _* v1.1.1-1 released_
...
tagged like **v1.1.1-1** (mind the dash in the middle!) and
named like **xPack Meson Build v1.1.1-1** (mind the dash),
...
- commit the `develop` branch of `xpack/web-jekyll` GitHub repo;
  use a message like _xPack Meson Build v1.1.1-1 released_
- push to GitHub
...
- commit all changes, use a message like
  _package.json: update urls for 1.1.1-1.1 release_ (without _v_)

## Publish on the npmjs.com server

- select the `xpack-develop` branch
- check the latest commits `npm run git-log`
- update `CHANGELOG.md`, add a line like _* v1.1.1-1.1 published on npmjs.com_
- commit with a message like _CHANGELOG: publish npm v1.1.1-1.1_
- `npm pack` and check the content of the archive, which should list
  only the `package.json`, the `README.md`, `LICENSE` and `CHANGELOG.md`;
  possibly adjust `.npmignore`
- `npm version 1.1.1-1.1`; the first 4 numbers are the same as the
...
- `npm dist-tag add @xpack-dev-tools/meson-build@1.1.1-1.1 latest`
- `npm dist-tag ls @xpack-dev-tools/meson-build`

In case the previous version is not functional and needs to be unpublished:

- `npm unpublish @xpack-dev-tools/meson-build@1.1.1-1.1`
...
- paste the release name like **xPack Meson Build v1.1.1-1 released**
