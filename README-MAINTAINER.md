...
Determine the version (like `0.62.2`) and update the `scripts/VERSION`
file; the format is `0.62.2-1`. The fourth number is the xPack release number
...
and fix them; assign them to a milestone (like `0.62.2-1`).
...
- add a new entry like _* v0.62.2-1 prepared_
- commit with a message like _prepare v0.62.2-1_
...
```sh
.../xpack-meson-build-0.62.2-1/bin/meson-build --version
0.62.2
```
...
- in `CHANGELOG.md`, add the release date and a message like _* v0.62.2-1 released_
...
tagged like **v0.62.2-1** (mind the dash in the middle!) and
named like **xPack Meson Build v0.62.2-1** (mind the dash),
...
- commit the `develop` branch of `xpack/web-jekyll` GitHub repo;
  use a message like _xPack Meson Build v0.62.2-1 released_
- push to GitHub
...
- commit all changes, use a message like
  _package.json: update urls for 0.62.2-1.1 release_ (without _v_)

## Publish on the npmjs.com server

- select the `xpack-develop` branch
- check the latest commits `npm run git-log`
- update `CHANGELOG.md`, add a line like _* v0.62.2-1.1 published on npmjs.com_
- commit with a message like _CHANGELOG: publish npm v0.62.2-1.1_
- `npm pack` and check the content of the archive, which should list
  only the `package.json`, the `README.md`, `LICENSE` and `CHANGELOG.md`;
  possibly adjust `.npmignore`
- `npm version 0.62.2-1.1`; the first 4 numbers are the same as the
...
- `npm dist-tag add @xpack-dev-tools/meson-build@0.62.2-1.1 latest`
- `npm dist-tag ls @xpack-dev-tools/meson-build`

In case the previous version is not functional and needs to be unpublished:

- `npm unpublish @xpack-dev-tools/meson-build@0.62.2-1.1`
...
- paste the release name like **xPack Meson Build v0.62.2-1 released**
